...
impl Adc {
    ...
    where
        A: Into<Option<&'b mut AdcPin<'c, N, P>>>,
    {
    ...
    pub fn diff_channel<'a, 'b, 'c, 'd, 'e, P, N, const NP: PortName, const NN: PortName, const PP: PinNum, const PN: PinNum>(
        ...
        pos: P,
        neg: N,
    ) -> Result<DiffChannel<'a, 'b, 'c, 'd, 'e, NN, NP, PP, PN>, ()>
    where
        P: Into<Option<&'b mut AdcDiffPPin<'c, NN, PP>>>,
        N: Into<Option<&'d mut AdcDiffMPin<'e, NP, PN>>>,
    {
...
impl<'a, 'b, 'c, 'd, 'e, const NP: PortName, const NN: PortName, const PP: PinNum, const PN: PinNum> DiffChannel<'a, 'b, 'c, 'd, 'e, NP, NN, PP, PN> {
    fn new(
        adc: &'a mut Adc,
        ch: u8,
        pos: Option<&'b mut AdcDiffPPin<'c, NP, PP>>,
        neg: Option<&'d mut AdcDiffMPin<'e, NN, PN>>,
    ) -> Result<DiffChannel<'a, 'b, 'c, 'd, 'e, NP, NN, PP, PN>, ()> {
        match (adc.id, ch, pos.as_ref().map(|_| (NP, PP)), neg.as_ref().map(|_| (NN, PN))) {
            // ADC0
            // e20:0-D0,0P
            // e21:0-D0,0M
